...
export default function QuestionnaireClient() {
  ...
  const handleDistanceChange = async (newDistance: number) => {
    ...
    if (scoredResults.length > 0) {
      try {
        // Get coords from current answers
        const query = await processQuestionnaire(answers)
        // Results are now handled server-side only
        setRecommendations({ bestNearby: [], closestAlt: [] })
      } catch (error) {
        console.error('Error recomputing results:', error)
      }
    }
  }
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          role="button"
                          onClick={() => setAnswers(prev => ({ ...prev, locationPreference: opt.key }))}
                ...
                <div className="text-center mt-[40px] md:mt-[56px]">
                  ...
                  <div className="max-w-3xl mx-auto space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 max-w-2xl mx-auto">
                      <button
                        type="button"
                        onClick={() => setAnswers(prev => ({ ...prev, hasDiagnosis: true }))}
                      ...
                      <button
                        type="button"
                        onClick={() => setAnswers(prev => ({ ...prev, hasDiagnosis: false, diagnosis: '' }))}
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <textarea
                              ...
                              onChange={(e) => setAnswers(prev => ({ ...prev, diagnosis: e.target.value }))}
