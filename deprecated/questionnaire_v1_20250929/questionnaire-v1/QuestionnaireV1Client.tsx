...
import { useQuestionnaireV1, QuestionnaireV1Answers, featureFlags } from "./QuestionnaireV1Context"
  ...
  const validateStep = (stepIndex: number, answers: QuestionnaireV1Answers): string | null => {
    switch (stepIndex) {
      ...
      case STEP_V1.AVAILABILITY:
        if (answers.availability.length === 0) return "Vyber alespoň jeden čas."
      ...
      case STEP_V1.SPECIAL_NEEDS:
        if (answers.ageGroups.length === 0) return "Vyber pro koho hledáš."
  ...
  const handleNext = async () => {
    ...
    if (currentStep < STEPS_V1.length - 1) {
      ...
      try {
        ...
        if (searchCriteria.ageGroups.length > 0) params.set('ageGroups', searchCriteria.ageGroups.join(','))
        if (searchCriteria.workplaceAccessibility.length > 0) params.set('workplaceAccessibility', searchCriteria.workplaceAccessibility.join(','))
          ...
          <div className="w-full flex items-center justify-center min-h-screen">
            ...
            <div className="w-full max-w-[920px] mx-auto mt-4 min-h-40 md:min-h-44" aria-live="polite">
              ...
              <div className="min-h-9 mb-4 flex items-center justify-center">
                {answers.conditionsMain.length > 1 && (
                  <div
                    role="tablist"
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        ...
                        const isSelected = answers.availability.includes(slot.key)
                        return (
                          <button
                            key={slot.key}
                            onClick={() => {
                              const updated = isSelected
                                ? answers.availability.filter(a => a !== slot.key)
                                : [...answers.availability, slot.key]
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-2xl mx-auto">
                      {[
                        ...
                        const isSelected = answers.ageGroups.includes(age.key)
                        return (
                          <button
                            key={age.key}
                            onClick={() => {
                              const updated = isSelected
                                ? answers.ageGroups.filter(a => a !== age.key)
                                : [...answers.ageGroups, age.key]
                  ...
                  <div>
                    ...
                    <div className="flex justify-center gap-4">
                      {[
                        ...
                        const isSelected = answers.workplaceAccessibility.includes(access.key)
                        return (
                          <button
                            key={access.key}
                            onClick={() => {
                              const updated = isSelected
                                ? answers.workplaceAccessibility.filter(a => a !== access.key)
                                : [...answers.workplaceAccessibility, access.key]
